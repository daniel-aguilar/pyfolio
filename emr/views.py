...
class MedicalRecordCreate(LoginRequiredMixin, View):
    ...
    def get(self, request, patient_pk):
        ...
        return render(
            request, self.template_name, {"form": self.form_class(), "patient": patient}
        )
    ...
    def post(self, request, patient_pk):
        ...
        if form.is_valid():
            ...
            return render(
                request, self.template_name, {"form": form, "patient": patient}
            )
...
class DataTablesLanguageFile(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if get_language() == "es":
            with (
                files("emr.resources.datatables")
                .joinpath("spanish.json")
                .open("r") as language_file
            ):
